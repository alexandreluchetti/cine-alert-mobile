...
| HTTP | Dio 5 + `AuthInterceptor` (Bearer + refresh automático em 401 com fila de requisições concorrentes via `Completer`) |
...
`AuthInterceptor` em `lib/core/network/dio_client.dart` usa um `Completer<String?>?` para lidar com múltiplas requisições `401` concorrentes:

- **Primeira requisição 401:** cria o `Completer`, executa `POST /auth/refresh`, salva o novo token e chama `complete(newToken)` — desbloqueando todas as demais.
- **Requisições 401 subsequentes (durante o refresh):** aguardam `_refreshCompleter!.future` e retentam com o token recebido.
